...
#Como a base tinha muitos registros vazios, decidimos substituir os NAs por zero para facilitar a modelagem.

#Transformando NAs em zero
dados_sem_na <- dados[-1]
dados_sem_na[is.na(dados_sem_na)] <- 0

#Na primeira tentativa de rodar um modelo linear com os dados, recebemos como resultado o erro de que havia variáveis com singularidades, o que não era permitido pelo modelo.
#Pesquisando, vimos que esse ocorre quando há variáveis explicativas que tem poucos valores únicos, e consequentemente pouca variabilidade.
#Por esse motivo, decidimos selecionar variáveis a partir de um critério de quantidade de valores únicos: variáveis com mais de 20 valores únicos.

#Analisando a qtd de níveis das variáveis
...
write.table(x,"E:/MBA FGV/Modelagem Estatística Avançada/Teste.txt")
...
#Com esse método, ficamos com 721 variáveis explicativas passíveis de entrar no modelo, e o modelo linear passou a rodar.
#No entanto, no summary do modelo vimos que boa parte das variáveis explicativas ficaram com NA em parâmetro estimado, Desvio Padrão e Teste t.
#Por esse motivo, decidimos avançar com uma abordagem mais sofisticada para redução de dimensionalidade.
#A técnica escolhida foi a Análise de Componentes Principais. Este método agrupa variáveis a partir de sua covariâncias, gerando combinações lineares das variáveis, que se chamam Componentes Principais.
